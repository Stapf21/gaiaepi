...
class ConfiguracoesController extends Controller
{
    ...
    public function updateBranding(UpdateBrandingRequest $request): RedirectResponse
    {
        ...
        if ($request->hasFile('logo')) {
            ...
            $updates['branding.logo_url'] = null;
        }
        ...
        if ($request->hasFile('favicon')) {
            ...
            $updates['branding.favicon_url'] = null;
        }
        ...
        if ($request->boolean('clear_login_overlay_gif')) {
            $oldOverlayPath = SystemSettingRepository::getValue('branding.login_overlay_gif_path');
            if ($oldOverlayPath && $disk->exists($oldOverlayPath)) {
                $disk->delete($oldOverlayPath);
            }
            $updates['branding.login_overlay_gif_path'] = null;
            $updates['branding.login_overlay_gif_url'] = null;
        }
        ...
        if ($request->hasFile('login_overlay_gif')) {
            $path = $request->file('login_overlay_gif')->store('branding', 'public');
            $oldOverlayPath = SystemSettingRepository::getValue('branding.login_overlay_gif_path');
            if ($oldOverlayPath && $disk->exists($oldOverlayPath)) {
                $disk->delete($oldOverlayPath);
            }
            $updates['branding.login_overlay_gif_path'] = $path;
            $updates['branding.login_overlay_gif_url'] = null;
        }
        ...
        if ($request->has('login_overlay_gif_url')) {
            $overlayGifUrl = trim((string) $request->input('login_overlay_gif_url', ''));
            $normalizedOverlayGifUrl = $overlayGifUrl !== '' ? $overlayGifUrl : null;
            $currentOverlayGifUrl = SystemSettingRepository::getValue('branding.login_overlay_gif_url');

            if ($normalizedOverlayGifUrl !== $currentOverlayGifUrl) {
                $updates['branding.login_overlay_gif_url'] = $normalizedOverlayGifUrl;
            }
        }

        if ($request->has('logo_url')) {
            $logoUrl = trim((string) $request->input('logo_url', ''));
            $normalizedLogoUrl = $logoUrl !== '' ? $logoUrl : null;
            $currentLogoUrl = SystemSettingRepository::getValue('branding.logo_url');

            if ($normalizedLogoUrl !== $currentLogoUrl) {
                $updates['branding.logo_url'] = $normalizedLogoUrl;
            }
        }

        if ($request->has('favicon_url')) {
            $faviconUrl = trim((string) $request->input('favicon_url', ''));
            $normalizedFaviconUrl = $faviconUrl !== '' ? $faviconUrl : null;
            $currentFaviconUrl = SystemSettingRepository::getValue('branding.favicon_url');

            if ($normalizedFaviconUrl !== $currentFaviconUrl) {
                $updates['branding.favicon_url'] = $normalizedFaviconUrl;
            }
        }
        ...
        $metadata = [
            ...
            'branding.login_overlay_gif_path' => [
                'label' => 'GIF de sobreposicao do login',
                'meta' => ['type' => 'file', 'accept' => 'image/gif,image/webp'],
            ],
            'branding.login_overlay_gif_url' => [
                'label' => 'URL do GIF de sobreposicao do login',
                'meta' => ['type' => 'url'],
            ],
        ];
